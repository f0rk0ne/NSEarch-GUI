...
if [ -f /etc/lsb-release ] || [ -f /etc/debian_version ] ; then
  ...
  apt-get install unzip libreadline-gplv2-dev build-essential checkinstall unzip sqlite3 libsqlite3-dev python3-pyqt5.qtwebkit -y  
  exit
  if [[ $nmapversion ]]; then
  ...
  printf "[+] Checking Dependencies for $os ($arch $kernel)....\n"
  yum install zlib-devel bzip2-devel openssl-devel ncurses-devel sqlite-devel readline-devel tk-devel gdbm-devel db4-devel libpcap-devel xz-devel python3-pyqt5.qtwebkit -y
  if [[ $nmapversion ]]; then
    printf "\n[+] Nmap already installed :D \n"
  else
    echo "[+] Installing nmap .... "
