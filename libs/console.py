...
class Console(cmd.Cmd):
    ...
    update_options = ['db', 'scripts']
    run_options = ['net', 'template', 'templates']
    ...
    def __init__(self):
        ...
        self.__hist_length = self.__helper.dbmodule.hist_len
    ...
    def help_history(self):
        ...
        self.command_help["history"] = {
            ...
            self.__helper.i18n.t(HELP_USAGE): usage
        }
    ...
    def help_doc(self):
        self.command_help["doc"] = {
            ...
            self.__helper.i18n.t(HELP_USAGE):
    ...
    def help_addfav(self):
        ...
        self.command_help["addfav"] = {
            ...
            "name": self.__helper.i18n.t(HELP_FAV_NAME),
            "ranking": self.__helper.i18n.t(HELP_FAV_RANKING),
            self.__helper.i18n.t(HELP_USAGE): usage
        }
    ...
    def help_modfav(self):
        usage = f'modfav name:http newranking:{self.great}'
        self.command_help["modfav"] = {
            "modfav": self.__helper.i18n.t("help.help_modfav"),
            "name": self.__helper.i18n.t(HELP_FAV_NAME),            
            "newranking": self.__helper.i18n.t(HELP_FAV_RANKING),
            self.__helper.i18n.t(HELP_USAGE): usage
        }
    ...
    def do_run(self, args):
        ...
        nmap_helper = helpernmap.HelperNmap(args, self)
        nmap_helper.process()

    def complete_run(self, text, line, begidx, endidx):
        del line, begidx, endidx
        if not text:
            commands = self.run_options[:]
        else:            
            commands = [
                f for f in self.run_options
                if f.startswith(text)
            ]
        return commands

    def help_run(self):
        usage = 'run net:127.0.0.1 template:default'
        usage += f' {self.or_str} run templates'
        self.command_help['run'] = {
            'run': self.__helper.i18n.t("help.help_run"),
            'net': self.__helper.i18n.t("help.help_run_net"),
            'template': self.__helper.i18n.t("help.help_run_template"),
            'templates': self.__helper.i18n.t("help.help_run_templates"),
            self.__helper.i18n.t("help.help_usage"): usage
        }        

    def do_update(self, args):
        update = self.__helper(args, "update", parent=self)
        update.process()
    ...
    def complete_update(self, text, line, begidx, endidx):
        del line, begidx, endidx
        if not text:
            commands = self.update_options[:]
        else:
            commands = [
                f for f in self.update_options
                if f.startswith(text)
            ]
        return commands

    def help_update(self):
        usage = f'update db {self.or_str} update scripts'
        self.command_help['update'] = {
            'update': self.__helper.i18n.t("help.help_update"),
            'db': self.__helper.i18n.t("help.help_updatedb"),
            'scripts': self.__helper.i18n.t("help.help_update_scripts"),
            self.__helper.i18n.t(HELP_USAGE): usage
        }
